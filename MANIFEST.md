...
В скрипте используется папка «Загрузки» текущего пользователя:

```powershell
$base = Join-Path $env:USERPROFILE "Downloads"
```

Чтобы сортировать другую папку, замените `$base` в начале скрипта на нужный путь.
...
После первого запуска в папке «Загрузки» (`%USERPROFILE%\Downloads`) появляются (или остаются) папки:
